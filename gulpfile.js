...
const gulp = require('gulp'), // Подключаем Gulp
  ...
  importFile = require('gulp-file-include'), // Импорт файлов (_include('path/to/file'))
...
gulp.task('scripts', function () {
  return gulp.src('app/js/libs.js')
    .pipe(importFile({ // 
      prefix: '_', // импортим все файлы, описанные в результируещем js
      basepath: '@root' // 
    }))
    .pipe(rename({
...
gulp.task('watch', ['browser-sync', 'pug', 'sass', 'scripts'], function () {
  ...
  gulp.watch('app/js/libs.js', ['scripts']); // Наблюдение за JS файлами в папке js
...
gulp.task('build', ['clean', 'img', 'css-production', 'scripts'], function () {
  ...
  gulp.src(['app/js/libs.min.js', 'app/js/common.js']) // Переносим скрипты в продакшен
